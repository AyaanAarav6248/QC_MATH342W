...
```{r}
expect_equal(sum(my_simple_ols_mode$e)/length(my_simple_ols_mod$e),sum(lm_mod$residuals), tol = 1e-4)
```
...
```{r}
expect_equal(g(my_simple_ols_obj_mod, mean(x)), mean(y))
```
...
Answer: b_0: 30.89381, b_1: 0.5445124 ( b_0 is higher and b_1 is less)
RMSE: 2.245934 ( This is more than Rsq )
Rsq: 0.2052434 ( This is less than Rsq)

How good is this model? How well does it predict? Discuss.
I believevt that this is the good model . we can prdict the future values. 
...
No, We can not predict that parents ans their chldren have the same height. However, it depends on genes. 

If they were to have the same height and any differences were just random noise with expectation 0, what would the values of beta_0 and beta_1 be?

Answer: beta_0 will be less than beta_1.
...
Answer: It happens because height tends to mean in this case. That's why, he called "Regression towards mediocrity in hereditary stature" which was later shortened to "regression to the mean".

Why should this effect be real?

Answer: This should be real because height abnormalities doesnt belong to carry on and it is regressed to mean height instead. When we talk about genetic, gene determines the height and if child is tall because it doesnt confirm that his generation's child will be taller .They could be shorter than him.

You now have unlocked the mystery. Why is it that when modeling with y continuous, everyone calls it "regression"? Write a better, more descriptive and appropriate name for building predictive models with y continuous.

Answer: When modeling with y continuous then everone calls it " regression" becasue height regressed to the mean and when it is mean oriented then it could be better than name in this scenerio.
