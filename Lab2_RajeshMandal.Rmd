...
for (i in 1:n){
  threshold = SL[i]
  num_errors = sum(as.numeric(SL>threshold) != y_logical) 
  num_errors_by_parameter[i,] = c(threshold, num_errors)  
  
  tst = data.frame(
    Sepalength = SL,
    thresh = rep(threshold, n),
    SL_greater_than_thresh = as.numeric(SL>threshold),
    y_logi= y_logical,
    col3_doesnt_match_y_logi_is_error = as.numeric(as.numeric(SL>threshold) != y_logical),
    sum_of_errors= rep(num_errors,n)
                   )
  print(tst)
}
...
of course, I believe that the summary makes sense in this case. Then, it fits given within setosa's value as long as the given below most of the versicolor's values.
